...
export function FlashCardsTab({flashCards, documentContextForChat}: TFlashCardsTabProps) {
  ...
  const selectedData = localFlashCards.find(fc => fc.id === selectedFlashCardId)?.data || [];
       ...
       <div className="flex-1 flex w-full h-full justify-center items-center py-10 max-w-4xl mx-auto flex-col gap-4">
           {showCounter ? (
             <div className="flex flex-col items-center justify-center gap-6 mt-8 w-full max-w-lg">
                <h3 className="text-xl font-semibold mb-4 text-center">How many Flash Cards do you want to generate?</h3>
                <FeatureCounter 
    ...
    return (
      <div className="min-w-full">
         <div className="max-w-full flex justify-start items-center px-6 mt-1 bg-blue-300/10 rounded-lg p-4 mx-5 shadow-xs">
           <Button variant="outline" size="sm" className="rounded-full shadow-md" onClick={() => setView('list')}>
  ...
  return (
    ...
    {flashCards.length === 0 ? (
      ...
      <div className="min-w-full">
        ...
        <Separator className="mt-4 mx-6 opacity-40" />
        {localFlashCards.length >= 5 && showLimitBanner && (
            <LimitReachBanner limitMessage="You have reached the limit of 5 flash card sets for this document." />
          )}
        <FlashCardsList 
