...
const plans: Plans[] = [
    {
        ...
        items: ['5 PDF summaries per week', 'Priority processing', '24/7 priority support', 'Markdown export'],
        paymentlink: "",
        priceId: "",
        icon: <User size={32} />
    },
...
const PricingCard = ({id, name, price, description, items, paymentlink, priceId, icon}: Plans) => {
    return (
        <div className="relative w-full max-w-lg hover:scale-105 hover:transition-all duration-200">
            ...
            <div className="flex gap-2">
                ...
                <div className="flex flex-col justify-end mb-[4px]"><p className="text-xs uppercase font-semibold">USD</p><p className="text-xs">/month</p></div>
            </div>
            <div className="space-y-2.5 leading-relaxed text-base flex-1">
                {
                    items.map((item, key) => (
                        <li key={key} className="flex items-center gap-2"><CheckIcon size={18} /> {item}</li>
                    ))
                }
            </div>
            <div className="space-y-2 flex justify-center w-full text-white">
                <Link href={"/paymentLink"} className={cn("w-full rounded-full flex items-center justify-center gap-2 bg-linear-to-r from-rose-800 to-rose-500 hover:from-rose-500 hover:to-rose-800 transform transition duration-1000 ease-in-out text-border-2 py-2", id==='pro' ? 'border-rose-900': 'border-rose-100 from-rose-400 to-rose-500')}>Buy Now <ArrowRight size={18} /> </Link>
            </div>
            </div>
        </div>
    )
