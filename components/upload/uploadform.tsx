...
export default function UploadForm() {
  ...
  const { startUpload, routeConfig } = useUploadThing("pdfUploader", {
    ...
    onUploadError: (er) => {
      ...
      toast("error occured while uploading",{
        description: "Due to some error you were unable to upload the file, please try again",
      })
    },
    onUploadBegin: (fileNameee) => { // so there is no specific name required for this 
  ...
  const handlesubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if(!validatedFiles.success) {
        console.log(validatedFiles.error.flatten().fieldErrors.file?.[0] ?? "Invalid File");
        toast("something went wrong",{
          description: "the files could not be validated according to defined parameters"
        })
        setLoading(false)
        return
      }
      ...
      if(data) {
        ...
        if(data.summary){
          ...
          console.log("following content to be sent as params in storeSummary : ");          
          console.log(`resp[0].name : ${resp[0].name}, data.title : ${data.title}, data.summary : ${data.summary}, resp[0].ufsUrl : ${resp[0].ufsUrl}`);
          ...
          console.log(`storeResult.data.id from uploadform.tsx : ${storeResult.data.id}`);
  ...
  return (
    <div className="flex flex-col gap-8 w-full max-w-2xl mx-auto my-5">
      {summariesCount < 5 ? <UploadFormInput isLoading={loading} ref= {formRef} onsubmit={handlesubmit} /> : <LimitCountBar />}
    </div>
  );
}
